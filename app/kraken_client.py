...
class KrakenClient:
    ...
    def __init__(self, api_key: str, api_secret_b64: str) -> None:
        ...
        if api_secret_b64:
            try:
                self.api_secret = base64.b64decode(api_secret_b64, validate=True)
            except BinasciiError as exc:
                raise ValueError(
                    "Invalid KRAKEN_API_SECRET: value must be the base64-encoded secret from Kraken. "
                    "Re-copy the secret exactly as provided by Kraken (no extra spaces or quotes)."
                ) from exc
