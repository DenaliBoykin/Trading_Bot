...
If you see:
`Invalid KRAKEN_API_SECRET` or a `binascii.Error` about base64,
your `KRAKEN_API_SECRET` in `.env` is malformed.

Fix:
1. Re-copy the Kraken API secret exactly (no quotes, no extra spaces).
2. Keep `DRY_RUN=true` while validating setup.
